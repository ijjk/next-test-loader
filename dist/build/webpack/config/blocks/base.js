"use strict";exports.__esModule=true;exports.base=void 0;var _lodash=_interopRequireDefault(require("next/dist/compiled/lodash.curry"));function _interopRequireDefault(obj){return obj&&obj.__esModule?obj:{default:obj};}const base=(0,_lodash.default)(function base(ctx,config){config.mode=ctx.isDevelopment?'development':'production';config.name=ctx.isServer?'server':'client';config.target=ctx.isServer?'node':'web';// Stop compilation early in a production build when an error is encountered.
...
if(ctx.isDevelopment){if(process.env.__NEXT_TEST_MODE&&!process.env.__NEXT_TEST_WITH_DEVTOOL){config.devtool=false;}else if(process.platform==='win32'){// Non-eval based source maps are slow to rebuild, so we only enable
