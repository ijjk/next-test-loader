...
async function hasNecessaryDependencies(baseDir, checkTSDeps, checkESLintDeps, lintDuringBuild = false) {
    ...
    let requiredPackages = checkESLintDeps ? requiredLintPackages : requiredTSPackages;
    ...
    const removalLintMsg = `\n\n` + (lintDuringBuild ? `If you do not want to run ESLint during builds, disable it in next.config.js. See https://nextjs.org/docs/api-reference/next.config.js/ignoring-eslint` : `Once installed, run ${_chalk.default.bold.cyan('next lint')} again.`);
    const removalMsg = checkTSDeps ? removalTSMsg : removalLintMsg;
    throw new _fatalError.FatalError(_chalk.default.bold.red(checkTSDeps ? `It looks like you're trying to use TypeScript but do not have the required package(s) installed.` : `To use ESLint, additional required package(s) must be installed.`) + '\n\n' + _chalk.default.bold(`Please install ${_chalk.default.bold(packagesHuman)} by running:`) + '\n\n' + `\t${_chalk.default.bold.cyan((isYarn ? 'yarn add --dev' : 'npm install --save-dev') + ' ' + packagesCli)}` + removalMsg + '\n');
}
