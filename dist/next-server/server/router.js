...
if(isCustomRoute&&parsedUrl.query.__nextLocale){if(keepBasePath){currentPathname=replaceBasePath(this.basePath,currentPathname);}currentPathname=`/${parsedUrl.query.__nextLocale}${currentPathname==='/'?'':currentPathname}`;if(keepBasePath){currentPathname=`${this.basePath}${currentPathname}`;}}const newParams=testRoute.match(currentPathname);// Check if the match function matched
